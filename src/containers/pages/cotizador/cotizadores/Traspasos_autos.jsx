...
import Data from '../datos_cotizador/traspaso_automotores.json';
import numeral from 'numeral';
import './Cotizador.css';
import { useMediaQuery } from "react-responsive";

const Traspasos_autos = () => {
  ...
  const calcularCotizacion = () => {
    ...
    const cotizacionesOrdenadas = Data.traspaso_automotores.valores
      .map(traspaso => ({
        ...traspaso,
        diferencia: Math.abs(valorFiscalNumerico - convertirAValorNumerico(traspaso.valor_fiscal_automotor)),
      }))
    ...
    // console.log(`Cotización Aproximada: ${cotizacionesOrdenadas[0].valor_fiscal_automotor}`);
    toggleTabla();
  };
              ...
              <table style={{
                ...
                <tbody>
                  {cotizacionesCercanas.map((cotizacion, index) => (
                    <tr key={index}>
                      <td>{formatNumber(cotizacion.valor_fiscal_automotor)}</td>
                      ...
                      <td>{formatNumber(cotizacion.subtotal_honorarios)}</td>
                      <td>{formatNumber(cotizacion.iva)}</td>
                      <td>{formatNumber(cotizacion.valor_traspaso_colones)}</td>
                      <td>{formatNumber(cotizacion.valor_traspaso_dolares)}</td>
                    </tr>
                  ))}
      ...
      {mostrarModal && (
        <div className="modal" onClick={cerrarModal}>
          <div className="modal-content rounded-shadow"
            ...
            <h2>Traspasos de Automotores</h2>
            <table>
              <thead>
                <tr>
                  <th>Valor Fiscal Automotor</th>
              ...
              <tbody>
                {Data.traspaso_automotores.valores.map((traspaso, index) => (
                  <tr key={index}>
                    <td>{traspaso.valor_fiscal_automotor}</td>
                    <td>{traspaso.timbres_registro}</td>
                    <td>{traspaso.honorarios_base}</td>
                    <td>{traspaso.declaracion_jurada}</td>
                    <td>{traspaso.subtotal_honorarios}</td>
                    <td>{traspaso.iva}</td>
                    <td>{traspaso.valor_traspaso_colones}</td>
                    <td>{traspaso.valor_traspaso_dolares}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
export default Traspasos_autos;
