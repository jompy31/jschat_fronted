...
import categoriesData from "../../../json/category.json"; // Import the category.json file
...
const CreateSubproduct = ({
  ...
  const getSubsubcategories = () => {
    if (!subproductSubcategory) return [];
    const selectedSubcategory = categories
      .flatMap((cat) => cat.subcategorias)
      .find((sub) => sub.nombre === subproductSubcategory);
    return selectedSubcategory ? selectedSubcategory.subsubcategorias : [];
  };
  ...
  // Actualizar previsualización cuando se edita un subproducto
  useEffect(() => {
  ...
  // Limpiar URLs de previsualización al cerrar el modal
  useEffect(() => {
    return () => {
  ...
  // Función para determinar si un archivo es una imagen basado en su extensión
  ...
  const validateForm = () => {
    ...
    if (subproductSubcategory && !subcategories.includes(subproductSubcategory)) {
      setError("La subcategoría seleccionada no es válida.");
      return false;
    }
    // Validate subsubcategory
    if (subproductSubsubcategory && !getSubsubcategories().includes(subproductSubsubcategory)) {
      setError("La sub-subcategoría seleccionada no es válida.");
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    setError("");
    try {
      editingSubproduct ? await handleEditSubproducts() : await handleSaveSubproducts();
    } catch (err) {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          <form className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* File Uploads con previsualización */}
            <div>
                ...
                <div className="mt-2">
                  {(filePreview.startsWith("blob:") || isImageFile(filePreview)) ? (
                    ...
                    <p className="text-sm text-gray-500">
                      Archivo seleccionado: {editingSubproduct?.file?.split("/").pop() || "Nuevo archivo"}
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="md:col-span-2">
             ...
             <div>
              ...
              <select
                ...
                onChange={(e) => setSubproductSubcategory(e.target.value)}
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setSubproductSubsubcategory(e.target.value)}
                disabled={!subproductSubcategory}
              >
