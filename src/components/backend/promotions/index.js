...
import promotionsData from '../../../containers/pages/Promociones/promotions.json';
import 'swiper/css';
...
const PromocionesAdmin = () => {
  ...
  const cardVariants = { hidden: { opacity: 0, scale: 0.95 }, visible: (i) => ({ opacity: 1, scale: 1, transition: { duration: 0.5, delay: i * 0.2 } }) };

  // Mapear imágenes según ID
  ...
  const handleCreate = (newPromo) => setPromotions([...promotions, { id: Date.now(), ...newPromo }]);
  const handleEdit = (id, updatedPromo) => setPromotions(promotions.map(p => p.id === id ? { ...p, ...updatedPromo } : p));
  const handleDelete = (id) => setPromotions(promotions.filter(p => p.id !== id));
    ...
    <div className="promociones-container">
      ...
      >
        ...
        <div className="relative z-10 text-center px-4">
          ...
          <a href="/contacto" className="bg-jsport-red hover:bg-red-600 text-white font-bold py-3 px-6 rounded-lg inline-flex items-center">
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-12 text-jsport-blue">Ofertas Destacadas</h2>
          <Swiper
            ...
            {promotions.map((promo) => (
              <SwiperSlide key={promo.id}>
                <div className="bg-gray-100 p-6 rounded-lg shadow-md">
                  <img src={getPromoImage(promo.id)} alt={promo.name} className="w-full h-48 object-cover rounded-lg mb-4" />
                  <h3 className="text-xl font-bold mb-2 text-jsport-blue">{promo.name}</h3>
                  <p className="text-gray-600 mb-4">{promo.description}</p>
                  <p className="text-sm text-gray-500">Descuento: {promo.discount}% | Compra mínima: ${promo.min_amount}</p>
                  <a href="/login" className="mt-4 inline-flex items-center text-jsport-red hover:text-red-600 font-semibold">
      ...
      {/* Grid Todas las Promociones */}
      <section className="py-16 bg-gray-100">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-12 text-jsport-blue">Todas las Promociones</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {promotions.map((promo, index) => (
              ...
              >
                ...
                <h3 className="text-xl font-bold mb-2 text-jsport-blue">{promo.name}</h3>
                <p className="text-gray-600 mb-4">{promo.description}</p>
                <p className="text-sm text-gray-500 mb-4">Descuento: {promo.discount}% | Compra mínima: ${promo.min_amount}</p>
                <div className="mb-4">
                ...
                <a href="/login" className="inline-flex items-center bg-jsport-red hover:bg-red-600 text-white font-bold py-2 px-4 rounded-lg">
      ...
      <section className="py-16 bg-jsport-blue text-white">
        <div className="container mx-auto px-4 text-center">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="text-xl mb-8">
